...
def read_xml3(path):
    """通过BeautifulSoup获取，忽略 translatable="false" 的条目
    ...
    for string in strings:
        key = string.get('name')
        translatable = string.get('translatable', 'true')  # 默认为 'true'，如果没有该属性

        if translatable == 'false':  # 如果 translatable 为 'false'，跳过此条目
            continue
    ...
    if not os.path.exists(dest_file_path):
        ...
        for _, dir_names, _ in os.walk(file_dir):
            ...
            for dir_name in values_dirs:
                ...
                index += 1
        workbook.save(dest_file_path)
    print(f"保存Excel文件至 {dest_file_path}")
    # 在生成文件后调用 compare_translations 进行漏翻译检查
    untranslated_output_path = dest_dir + "/" + "untranslated_results.xls"
    print(f"开始漏翻译检查，输出路径为 {untranslated_output_path}")
    compare_translations(dest_file_path, untranslated_output_path)
...
# 将 compare_translations 函数添加到代码中
def compare_translations(file_path, output_path):
    print(f"对比文件路径: {file_path}")
    ...
    # 读取第二列作为参考列
    reference_col = [sheet.cell_value(rowx=i, colx=1) for i in range(num_rows)]

    # 创建新的工作簿用于保存对比结果
    result_workbook = Workbook()

    for col_index in range(2, num_cols):  # 从第三列开始
        current_col = [sheet.cell_value(rowx=i, colx=col_index) for i in range(num_rows)]
        sheet_name = current_col[0]

        untranslated_data = []
        for row_index in range(1, num_rows):  # 从第二行开始（跳过标题）
            ref_value = reference_col[row_index]
            current_value = current_col[row_index]
            if ref_value and not current_value:
                untranslated_data.append((sheet.cell_value(rowx=row_index, colx=0), ref_value))

        if untranslated_data:
            new_sheet = result_workbook.add_sheet(sheet_name)
            new_sheet.write(0, 0, "name")
            new_sheet.write(0, 1, "en")
            new_sheet.write(0, 2, current_col[0])
            print(f"语言: {current_col[0]}")
            print(f"未翻译条数: {len(untranslated_data)}")
            print(f"                         ")
            for row_index, (key, untranslated) in enumerate(untranslated_data, start=1):
                new_sheet.write(row_index, 0, key)
                new_sheet.write(row_index, 1, untranslated)
...
def start_convert(options):
    file_dir = '/Users/liboyan/Documents/aMessage/app/src/main/res'
    target_dir = '/Users/liboyan/Documents/toxls'
